...
def fetch_ga4_summary(start, end):
    ...
    dev = client.run_report(RunReportRequest(
        property=prop, date_ranges=[DateRange(start_date=start, end_date=end)],
        dimensions=[Dimension(name="deviceCategory")],
        metrics=[Metric(name="sessions")],
        order_bys=[OrderBy(metric=OrderBy.MetricOrderBy(metric_name="sessions"), desc=True)],
    ))
    devices = [{"device": r.dimension_values[0].value.title(), "sessions": int(r.metric_values[0].value)}
               for r in dev.rows]
...
# ── Row 3b: Device Breakdown ─────────────────────────────────────────────────
if ga4_ok and ga4_data and ga4_data.get("devices"):
    st.markdown(
...
# ── Row 3c: Conversion Rate by Source ────────────────────────────────────────
